...
def configure(openstack_config, nameserver=None):
    if nameserver:
      fabric.api.run('echo "nameserver {0}" | sudo tee --append /etc/resolv.conf > /dev/null'.format(nameserver))
    fabric.api.run('sudo yum -y update')
    fabric.api.run('sudo yum -y install docker')
